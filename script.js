...
function renderPublications(list){
  ...
  host.innerHTML=completeList.map(p=>{
    ...
    return `<a class="pub-row ${v.src?'has-visual':'no-visual'}" href="${safeUrl(p.url||'#')}" target="_blank" rel="noopener">${media}<span class="pub-year">${safeText(p.year||'—')}</span><span class="pub-title">${safeText(title)}${authors}${originalLine}</span><span class="pub-journal">${safeText(p.journal||'')}</span><span class="badges"><b class="badge ${badgeClass(qtile)}">${safeText(qtile)}</b><b class="badge ${ifClass}" title="${safeText(p.impact_factor_source||'')}">${safeText(iff)}</b>${cite}</span><span class="pub-arrow">↗</span></a>`
  }).join('');
